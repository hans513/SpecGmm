/* Hadamard Transform
   ...
   In terms of default matlab functions, this is functionally equivalent 
        to hadamard(size(x,1))*x or ifwht(x,[],'hadamard')
   ...
   Email: stobbe@caltech.edu
   Created: August 2008, edited comments September 2011
*/
...
void checkPowerTwo(unsigned m)
{
    ...
    if (m <= 1) {
        printf("Vector length must be greater than 1.");
    }
    ...
    if (m > 1) {
        printf("Vector length must be power of 2.");
    }
}
